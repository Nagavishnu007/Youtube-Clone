...
import { API_KEY, value_count } from '../../data'
    ...
    <>
      <div className='recommended'>
          ...
          {apiData.map((item,index)=>{
             return(
              <Link to={`/video/${item.snippet.channelTitle}/${item.id}`} key={index} className='side-video-list'>
              <img src={item.snippet.thumbnails.medium.url} alt='thumbnail1'/>
                <div className='video-info'>
                    <h4>{item.snippet.title}</h4>
                    <p>{item.snippet.channelTitle}</p>
                    <p>{value_count(item.statistics.viewCount)} views</p>
                </div>
            </Link>
             )
          })}
